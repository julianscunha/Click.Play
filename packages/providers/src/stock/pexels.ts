...
import type { StockAsset, StockCandidate, StockProvider } from "./types.js";
...
export class PexelsStock implements StockProvider {
  ...
  constructor(apiKey?: string, cacheDir?: string) {
    ...
    this.cacheDir = cacheDir ?? path.join(process.env["HOME"] ?? "/tmp", ".clickplay", "cache", "stock");
  ...
  private pickVideoFile(files: PexelsVideoResponse["videos"][number]["video_files"]) {
    return (
      files.filter((f) => (f.width ?? 0) >= 720 && (f.width ?? 0) <= 1920).sort((a, b) => (b.width ?? 0) - (a.width ?? 0))[0] ??
      files.filter((f) => (f.width ?? 0) >= 720).sort((a, b) => (a.width ?? 0) - (b.width ?? 0))[0]
    );
  }
  ...
  async download(candidate: StockCandidate): Promise<StockAsset> {
    ...
    return { filePath, width: candidate.width, height: candidate.height, duration: candidate.duration };
  }
