...
- **10B — Resolução de asset.** `resolveElement(element: VisualElement, ctx) → ResolvedElement` (chama `ImageProvider`/`VideoGenerationProvider` a partir do `prompt`), fecha o contrato que a Fase 9 deixou em aberto — output compatível com `ResolvedElement` de `packages/video-engine/src/render/types.ts`.
