...
- **Fase 15 — Studio UX.** Wizard visual robusto (10-20min), telas: Briefing → Roteiro → Visual → Música/Som → Narração → Legendas → Efeitos/Transições → Abertura/Fechamento → Providers/Configuração → Revisão/Custo → Geração → QC → Publicação. Não é editor tipo Premiere/DaVinci, não é "prompt→gerar" puro. Depende só da Fase 11 (WebUI MVP) já existente; fazer depois de Projeto/Template (16-17) estarem no schema, pra já desenhar a tela "salvar como template" sabendo que a entidade existe.
  ...
  **Status (2026-08-22): 1ª rodada implementada — esqueleto do wizard + 4 telas core.** `apps/web/src/components/wizard/Wizard.tsx` substitui `CreateForm.tsx` (removido). Stepper lateral colapsável (só ícones quando recolhido), navegação por clique em passo já visitado, sem lib nova (só `useState`). Telas implementadas: **Briefing** (topic, direction), **Roteiro** (archetype, pacing, idioma pt-BR/en-US, duração-alvo em segundos — todos os campos do §11A Bloco 2/3 já existentes no backend, sem UI até agora), **Visual** (videoMode, aspectRatio, captionStyle, qualityTier — Bloco 2), **Revisão/Custo** (resumo somente-leitura + submit; custo real em USD segue mostrado na tela seguinte, `ProgressView`, via `AWAITING_COST_APPROVAL`, não duplicado aqui). `api.ts` `CreateJobInput` ganhou `qualityTier`/`targetDurationSeconds`/`language`. Layout decidido via mockup comparativo (Artifact) antes do código — usuário escolheu stepper lateral colapsável sobre horizontal. **Pendente**: telas Música/Som, Narração, Legendas (posição/toggle já tem backend, Bloco 4 item 6/7), Efeitos/Transições, Abertura/Fechamento (Bloco 5, backend já existe), Providers/Configuração (hoje só em `SettingsView` separado), QC, Publicação — próximas rodadas. Build de produção (`vite build`) e typecheck verificados; sem ferramenta de browser neste ambiente pra validação visual real.
...
**Gap descoberto (2026-08-22) — música e narração não têm dial de qualidade equivalente.** Checagem da mesma pergunta pra `MusicProvider`/`TTSProvider`:
- **Narração (TTS): nenhum dial de qualidade com custo variável existe.** `EdgeTTS` é grátis por natureza (serviço público Microsoft) — único "dial" é bitrate de output (`msedge-tts` `OUTPUT_FORMAT`: `AUDIO_24KHZ_48KBITRATE_MONO_MP3` fixo hoje vs `AUDIO_24KHZ_96KBITRATE_MONO_MP3`/Opus disponíveis), mas grátis nos dois, não afeta custo — ganho marginal de fidelidade, não vale tier. `OpenRouterTTS`/`GeminiTTS` (`SAMPLE_RATE = 24000` fixo no código) — a API real (`/v1/audio/speech`) não documenta parâmetro de qualidade/sample_rate configurável (diferente de imagem/vídeo que têm `resolution`); `TTS_PRICING_PER_CHAR` já é fixo por char, condizente com a API não ter dial. Conclusão: **TTS fica fora de `qualityTier` — não há o que variar**.
- **Música: custo varia por MODELO, não por dial dentro de 1 chamada.** `BundledMusic` é flat (grátis, arquivo estático). Lyria tem dois modelos com preço bem diferente — Lyria 3 **Clip** (~$0.04/30s, 48kHz stereo) vs Lyria 3 **Pro** (~$0.08/música até 3min, estruturado intro/verso/refrão/outro, é o que `music/lyria.ts` usa hoje fixo via `MODEL = "lyria-3-pro-preview"`). Proposta futura: `MODEL_BY_TIER` ganha entrada de música (draft→Clip, standard/high→Pro), mesmo padrão já usado pro LLM — não implementado ainda, decidir junto do gap de resolução acima.
...
**Tamanho/poucas palavras por vez (item 6).** **Status (2026-08-22): fix root-cause já implementado.** `orchestrator.ts:212-215` resolve `captionChunkSize`/`captionLingerS` a partir de `getArchetype(score.archetype)` como default, com `opts.captionChunkSize`/`opts.captionLingerS` tendo prioridade — mesma hierarquia de `captionStyle`. Teste cobrindo: `orchestrator.test.ts` ("falls back to the archetype's caption chunk/linger instead of a hardcoded default"). **Pendente, adiado pra Fase 15**: expor `captionChunkSize` (faixa "poucas/algumas/muitas palavras", não número cru) e posição (topo/centro/rodapé — motor de legendas não tem prop de posição hoje, os 7 estilos em `captions/styles/*.tsx` presumem posição fixa) na UI.

**Elementos de texto/tópico atrapalhando (item 7).** **Status (2026-08-22): implementado** (commit `0ff668b`) — `animated_text` ganhou `position` (`top`/`bottom`/`center`/`random`, resolvido em `resolve-element.ts`, "random" sorteado 1x na resolução) e `PipelineOptions.showTextOverlays` (quando `false`, instrui Creative Director generate+revise a não emitir `animated_text`). Seletor de UI pra `position`/`showTextOverlays` fica pra Fase 15, mesma decisão do Bloco 2/3.

**Placeholder vazando pro vídeo final (item 8).** **Status (2026-08-22): curativo (a) já implementado** (commit `15ce38e`) — `UnsupportedElement.tsx` é fallback silencioso (`AbsoluteFill` vazio) e `buildVideoModeGuidance`/`strategyGuidance` (`creative-director.ts:126,275`) já instruem o LLM a não usar svg/shape/icon/particle_system/diagram. (b) implementação real de cada tipo segue como débito aberto — escopo maior (reabertura da Fase 9), não cabe neste roadmap.
...
**Status (2026-08-22): modo `generated` implementado backend+API.** `IntroOutroConfig` (`packages/domain/src/scene.ts`) — `{ mode: "generated" | "upload", text?: string, transition?: TransitionType }` — exposto em `CreateJobBody.intro`/`.outro`. `resolveIntroOutroScene` (`pipeline/intro-outro.ts`) monta a `Scene` sintética (1 elemento `animated_text` posição "center" + `transition`, default "crossfade") e o orchestrator faz prepend (intro)/append (outro) no `score.scenes` logo após o loop de revisão do Director, antes do `costEstimate` — cai automaticamente no TTS/render/QC/custo real sem tocar mais nada no pipeline. Texto: `text` explícito do usuário tem prioridade; se vazio, 1 LLM call gera a partir do `research` já feito (prompt curto, sem retry). Modo `upload` só existe no enum do schema — API rejeita com 422 se `mode: "upload"` for enviado (`'intro/outro modo "upload" ainda não implementado'`); resolução real fica pra Fase 15 junto do endpoint de upload de arquivo (não existe hoje — API só serve output, nunca recebe asset do usuário), validação de formato/duração/aspect ratio compatível com o `RenderInput` do job, e composição no `RemotionRenderer` como cena de vídeo-fonte fixo. UI (seletor generated/upload, campo de texto) adiada pra Fase 15, mesma decisão do Bloco 2/3. Esforço realizado: M.

### Bloco 6 — Observabilidade e output final (itens 9, 11)

**Status (2026-08-22): item 11 metade backend implementada.** `currentStep` textual já existia de outro bugfix anterior (2026-08-16, `job.stageDetail`, "Gerando cena N/M (tipo)") — a proposta abaixo ficou desatualizada nesse ponto, não precisou de código novo. O que faltava (contagem final: `imageCount`/`videoClipCount`/`audioSeconds`) foi implementado: `PipelineResult` (status completed) e `jobs.result_summary` (nova coluna JSON) carregam os 3 campos, expostos em `GET /jobs/:id` como `resultSummary`. **Item 9 (download+card de custo) e a exibição de `resultSummary`/`currentStep` na tela seguem 100% pendentes — são só frontend (`ResultPlayer.tsx`/`ProgressView.tsx`), adiados pra Fase 15 (Studio UX) junto do resto da UI (mesma decisão do Bloco 2/3).**
...
**Status (2026-08-22): adiado inteiro pra Fase 15.** Decisão deliberada — diferente do Bloco 2/3 (campos backend úteis via API mesmo sem UI), aqui o valor inteiro do bloco é o botão no form; a rota sozinha (`POST /briefing/expand`) fica inerte sem UI pra chamá-la. Não vale construir agora e deixar parado até o redesign.
